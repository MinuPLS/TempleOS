...
export const ConnectedLiquidityPools = ({ tokenPrices }: ConnectedLiquidityPoolsProps) => {
      ...
      <div className={styles.liquidityPoolsHeader}>
        ...
        <div className={styles.liquidityPoolsActions}>
          <button
            ...
            aria-label={isInfoOpen ? 'Hide liquidity pool info' : 'Show liquidity pool info'}
            aria-pressed={isInfoOpen}
            title="About these pools"
          >
            <Info size={16} />
          </button>
          <button
            type="button"
            className={`${styles.activityRefreshButton}${isLoading ? ` ${styles.refreshSpinning}` : ''}`}
            onClick={() => void refresh()}
            disabled={isLoading}
            aria-label="Refresh pool balances"
